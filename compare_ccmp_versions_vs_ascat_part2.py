...
var_list = ['u','v','w']
var_name_list = ['Zonal Wind','Meridional Wind','Wind Speed']
        ...
        for no_sat in [False,True]:
            ...
            for  year in year_list:
                for month in np.arange(1,13,dtype=np.int):
                    ...
                    w_hist_1D_ccmp = Hist1D.from_netcdf(nc_file=nc_file, varname = f'n_{wind_var}_ccmp_{compare_sat}', name = f"{wind_var.upper()} CCMP at {compare_sat}")
            ...
            fig,ax = w_hist_2D_tot.plot(title=title_str,xtitle=f'{wind_var.upper()} ASCAT-B (m/s)', ytitle=f'{wind_var.upper()} CCMP (m/s)',fontsize=12)
