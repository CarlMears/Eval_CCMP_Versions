...
compare_sats = ['AMSR2']
has_vectors = False
ccmp_versions = ['era5_current_corrected','era5_current_corrected_adj2']
ccmp_subversions = ['v2.0','v2.0']
    ...
    for k,compare_sat in enumerate(compare_sats):
        ...
        for  year in year_list:
            for month in np.arange(1,13,dtype=np.int):
                ...
                var_list = [ccmp_vs_ascat_w_maps]
                if has_vectors:
                    var_list = [ccmp_vs_ascat_u_maps,ccmp_vs_ascat_v_maps,ccmp_vs_ascat_w_maps]
                for n,map in enumerate(var_list):
        ...
        num_obs = ccmp_vs_ascat_w_maps_tot.num_obs()
        ...
        num_fig,ax       = global_map(num_obs,         cmap='magma', vmin= 0.0, vmax=1000.0, plt_colorbar=True,title=f"{ccmp_version} - {compare_sat}, {start_year}-{end_year}, Number of Collocations")
        if has_vectors:
            umap_diff_fig,ax = global_map(mean_ccmp_u_diff, cmap='BrBG', vmin=-1.5, vmax=1.5, plt_colorbar=True,title=f"{ccmp_version} - {compare_sat}, {start_year}-{end_year}, U Bias (m/s)")
            vmap_diff_fig,ax = global_map(mean_ccmp_v_diff, cmap='BrBG', vmin=-1.5, vmax=1.5, plt_colorbar=True,title=f"{ccmp_version} - {compare_sat}, {start_year}-{end_year}, V Bias (m/s)")
        wmap_diff_fig,ax = global_map(mean_ccmp_w_diff, cmap='BrBG', vmin=-1.5, vmax=1.5, plt_colorbar=True,title=f"{ccmp_version} - {compare_sat}, {start_year}-{end_year}, W Bias (m/s)")
        ...
        if has_vectors:
            umap_diff_fig4,ax = global_map(std_ccmp_u_diff, cmap='magma', vmin=0.0, vmax=2.0, plt_colorbar=True,title=f"{ccmp_version} - {compare_sat}, {start_year}-{end_year},U Std. Dev. (m/s)")
            vmap_diff_fig4,ax = global_map(std_ccmp_v_diff, cmap='magma', vmin=0.0, vmax=2.0, plt_colorbar=True,title=f"{ccmp_version} - {compare_sat}, {start_year}-{end_year}, V Std. Dev. (m/s)")
        wmap_diff_fig4,ax = global_map(std_ccmp_w_diff, cmap='magma', vmin=0.0, vmax=2.0, plt_colorbar=True,title=f"{ccmp_version} - {compare_sat}, {start_year}-{end_year}, W Std. Dev. (m/s)")
        ...
        png_file = f"{plt_path}nobs_CCMP_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.png"
        num_fig.savefig(png_file)
        plt.close(num_fig)
        if has_vectors:
            png_file = f"{plt_path}mean_CCMP_U_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.png"
            umap_diff_fig.savefig(png_file)
            plt.close(umap_diff_fig)
            png_file = f"{plt_path}mean_CCMP_V_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.png"
            vmap_diff_fig.savefig(png_file)
            plt.close(vmap_diff_fig)
        png_file = f"{plt_path}mean_CCMP_W_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.png"
        wmap_diff_fig.savefig(png_file)
        plt.close(wmap_diff_fig)

        '''png_file = f"{plt_path}mean_CCMP_U_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.trop_pac.png"
        umap_diff_fig3.savefig(png_file)
        plt.close(umap_diff_fig3)
        png_file = f"{plt_path}mean_CCMP_V_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.trop_pac.png"
        vmap_diff_fig3.savefig(png_file)
        plt.close(vmap_diff_fig3)
        png_file = f"{plt_path}mean_CCMP_W_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.trop_pac.png"
        wmap_diff_fig3.savefig(png_file)
        plt.close(wmap_diff_fig3)
        '''

        if has_vectors:
            png_file = f"{plt_path}std_CCMP_U_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.png"
            umap_diff_fig4.savefig(png_file)
            plt.close(umap_diff_fig4)
            png_file = f"{plt_path}std_CCMP_V_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.png"
            vmap_diff_fig4.savefig(png_file)
            plt.close(vmap_diff_fig4)
        png_file = f"{plt_path}std_CCMP_W_{ccmp_version}_minus_{compare_sat}_{start_year}-{end_year}.png"
        ...
        if has_vectors:
            ...
            png_file = f"{plt_path}mean_CCMP_U_{ccmp_version}_minus_{compare_sat}_time_lat.png"
            ...
            ax.set_xlabel('Year')
            ax.set_title(f"{ccmp_subversion} minus {compare_sat}")
            png_file = f"{plt_path}mean_CCMP_V_{ccmp_version}_minus_{compare_sat}_time_lat.png"
            fig.savefig(png_file)
            plt.close(fig)
        ...
        ax.set_xlabel('Year')
        ax.set_title(f"{ccmp_subversion} minus {compare_sat}")
        png_file = f"{plt_path}mean_CCMP_W_{ccmp_version}_minus_{compare_sat}_time_lat.png"
        fig.savefig(png_file)
        plt.close(fig)
        ...
        if has_vectors:
            ...
            ax.set_xlabel('Year')
            ax.set_title(f"{ccmp_subversion} minus {compare_sat}")
            png_file = f"{plt_path}stddev_CCMP_U_{ccmp_version}_minus_{compare_sat}_time_lat.png"
            fig.savefig(png_file)
            plt.close(fig)
            ...
            ax.set_xlabel('Year')
            ax.set_title(f"{ccmp_subversion} minus {compare_sat}")
            png_file = f"{plt_path}stddev_CCMP_V_{ccmp_version}_minus_{compare_sat}_time_lat.png"
            fig.savefig(png_file)
            plt.close(fig)
        ...
        ax.set_xlabel('Year')
        ax.set_title(f"{ccmp_subversion} minus {compare_sat}")
        png_file = f"{plt_path}stddev_CCMP_W_{ccmp_version}_minus_{compare_sat}_time_lat.png"
        fig.savefig(png_file)
        plt.close(fig)
...
if has_vectors:
    var_list = ['U','V','W']
for ivar,var_name in enumerate(var_list):
    fig = plt.figure(figsize=(7, 7))
    ax = fig.add_subplot(111)
    for j,ccmp_version in enumerate(ccmp_versions):
        ccmp_subversion = ccmp_subversions[j] 

        for k,compare_sat in enumerate(['WindSAT']):
            if has_vectors:
                ax.plot(zonal_means[j,k,ivar,:],lats,label = f'{ccmp_version}-{compare_sat}')
            else:
                ax.plot(zonal_means[j,k,2,:],lats,label = f'{ccmp_version}-{compare_sat}')
    ...
    ax.set_xlabel = f"{var_name}, CCMP - {compare_sat}"
    ax.set_ylabel = 'Latitude'
    png_file = f"{plt_path}CCMP_minus_{compare_sat}_zonal_mean_{var_name}.png"
    fig.savefig(png_file)
print()
...
for ivar,var_name in enumerate(var_list):
    ...
    ax.set_ylim((-1.0,1.0))
    ax.legend()
    png_file = f"{plt_path}{var_name}_CCMP_minus_{compare_sat}_spatial_mean_ts.png"
    fig.savefig(png_file)
...
for ivar,var_name in enumerate(var_list):
    ...
    ax.set_ylim((0.0,1.0))
    ax.legend()
    png_file = f"{plt_path}{var_name}_CCMP_minus_{compare_sat}_spatial_stddev_ts.png"
    fig.savefig(png_file)
