...
compare_sats = ['WindSAT']
has_vectors = False
ccmp_versions = ['era5_current_corrected_adj2']
...
for ivar,var_name in enumerate(['U','V','W']):
    ...
    ax.set_ylabel = 'Latitude'
    png_file = f"{plt_path}CCMP_minus_ASCAT_zonal_mean_{var_name}.png"
    fig.savefig(png_file)
print()
...
for ivar,var_name in enumerate(['U','V','W']):
    ...
    ax.set_ylim((-1.0,1.0))
    ax.legend()
    png_file = f"{plt_path}{var_name}_CCMP_minus_ASCAT_spatial_mean_ts.png"
    fig.savefig(png_file)
...
for ivar,var_name in enumerate(['U','V','W']):
    ...
    ax.set_ylim((0.0,1.0))
    ax.legend()
    png_file = f"{plt_path}{var_name}_CCMP_minus_ASCAT_spatial_stddev_ts.png"
    fig.savefig(png_file)
